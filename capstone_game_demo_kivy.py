...
JUMP_VELOCITY = 750  # initial jump velocity - tuned for feel
...
SPAWN_INTERVAL_BASE = 1.9  # base interval between obstacles (sec) - modified by speed
...
OBSTACLE_PREVIEW_SECONDS = 5.5  # show next obstacle before background changes
...
class GameWidget(Widget):
    ...
    player_x = NumericProperty(350.0)  # horizontal position of player (constant)
    ...
    def __init__(self, embedded=False, **kwargs):
        ...
        self.obstacle_sources = {
            0: "images/obstacle_sea.png",
            1: "images/obstacle_forest.png",
            2: "images/obstacle_desert.png",
            3: "images/obstacle_sky.png",
            4: "images/obstacle_space.png",
        }

        self._spawn_pause_remaining = 0.0  # [NEW] spawn pause timer after background switch

        # backgrounds
        self.backgrounds = [
            "images/background_sea.png",
            "images/background_forest.png",
            "images/background_desert.png",
    ...
    def reset_game(self):
        ...
        self._bg_elapsed = 0.0
        self._slow_until = None
        self._spawn_pause_remaining = 0.0  # [NEW] reset spawn pause timer
    ...
    def spawn_obstacle(self):
        ...
        # If we are within N seconds of a background switch, spawn obstacles using the NEXT background's obstacle theme
        ...
        src = self.obstacle_sources.get(preview_index, "images/obstacle_default.png")
    ...
    def update(self, dt):
        ...
        if self.is_running and not self.game_over:
            ...
            if self._bg_elapsed >= self.BG_SWITCH_SECONDS:
                ...
                self.bg_source = self.backgrounds[self.bg_index]
                #self._spawn_pause_remaining = SPAWN_PAUSE_ON_BG_SWITCH  # pause spawning on bg switch
                #self._spawn_accumulator = 0.0  # prevent immediate spawn after pause
            ...
            if self._spawn_pause_remaining > 0.0:  # [NEW] spawn pause countdown
        ...
        with self.canvas.before:
            ...
            for ob in self.obstacles:
                Color(1, 1, 1, 1)  # [CHANGED] draw obstacle image (no tint)
                Rectangle(pos=(ob.x, ob.y), size=(ob.size, ob.size), source=ob.source)  # [CHANGED] use source
